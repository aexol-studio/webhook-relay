...
impl Config {
    pub async fn from_env() -> Result<Self> {
        let external_url = env::var("EXTERNAL_URL")
            .context("EXTERNAL_URL environment variable is required")?;
        ...
        let jwt_audience = env::var("JWT_AUDIENCE")
            .context("JWT_AUDIENCE environment variable is required")?;
        ...
        let (jwks_url, jwt_issuer) = if let Ok(oidc_issuer) = env::var("OIDC_ISSUER") {
            ...
            let jwks_url = env::var("JWKS_URL")
                .context("Either OIDC_ISSUER or JWKS_URL must be set")?;
